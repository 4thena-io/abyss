  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="bg-panel border border-b1 rounded-xl p-5">
        <div class="text-[11px] font-semibold uppercase tracking-wider text-t3 mb-2">Applications</div>
        <div class="text-3xl font-bold text-t1 tracking-tight">{{ apps.length }}</div>
        <div class="text-xs text-t3 mt-1">in this project</div>
      </div>
      <div class="bg-panel border border-b1 rounded-xl p-5">
        <div class="text-[11px] font-semibold uppercase tracking-wider text-t3 mb-2">Created by</div>
        <div class="text-lg font-semibold text-t1 mt-1 truncate">{{ project.creatorUsername || '—' }}</div>
      </div>
      <div v-if="project.teamId" class="bg-panel border border-b1 rounded-xl p-5">
        ...
        <RouterLink :to="`/teams/${project.teamId}`" class="text-lg font-semibold text-acc hover:opacity-80 mt-1 block truncate">
          Team #{{ project.teamId }}
        </RouterLink>
      </div>
    ...
    <div v-if="loading" class="grid grid-cols-2 gap-3">
      <div v-for="i in 2" :key="i" class="bg-panel border border-b1 rounded-xl p-4 h-28 animate-pulse" />
    </div>
      ...
      >
        <div class="flex items-center justify-between px-4 py-3 border-b border-b1 bg-surface">
          <div class="flex items-center gap-2">
            ...
            <span v-if="app.kind || app.language" class="text-[11px] px-1.5 py-0.5 rounded bg-surface border border-b1 text-t2 font-mono">
              {{ [app.language, app.kind].filter(Boolean).join(' · ') }}
            </span>
          </div>
          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-surface text-t2 border border-b1">
        ...
        <div class="px-4 py-3">
          ...
          <div class="flex items-center gap-2 mt-2">
            <a
              ...
              @click.stop
            >CI pipeline →</a>
            <span v-if="app.repoUrl" class="text-xs text-t3 font-mono truncate">{{ repoLabel(app.repoUrl) }}</span>
          </div>
        </div>
      </RouterLink>
    </div>
