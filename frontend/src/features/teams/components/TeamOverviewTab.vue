    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="bg-panel border border-b1 rounded-xl overflow-hidden">
        <div class="flex items-center justify-between px-4 py-3 border-b border-b1 bg-surface">
          ...
          <RouterLink :to="`/teams/${team.id}?tab=members`" class="text-xs text-acc hover:underline">
            View all →
          </RouterLink>
        </div>
        ...
        <div v-else-if="members.length === 0" class="px-4 py-6 text-sm text-t3 text-center">No members</div>
        <div v-else class="p-3 space-y-1">
          ...
          >
            <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-mono text-[11px] font-semibold"
              :class="m.role === 'owner' ? 'bg-acc-s border border-acc-b text-acc' : 'bg-surface border border-b1 text-t2'">
              {{ initials(m.username) }}
            </div>
            ...
            <span
              ...
              :class="m.role === 'owner' ? 'bg-ok-s text-ok' : 'bg-surface text-t2 border border-b1'"
            >{{ m.role }}</span>
          </div>
      ...
      <div class="bg-panel border border-b1 rounded-xl overflow-hidden">
        <div class="flex items-center justify-between px-4 py-3 border-b border-b1 bg-surface">
          ...
          <RouterLink :to="`/teams/${team.id}?tab=projects`" class="text-xs text-acc hover:underline">
            View all →
          </RouterLink>
        </div>
        ...
        <div v-else-if="projects.length === 0" class="px-4 py-6 text-sm text-t3 text-center">No projects</div>
