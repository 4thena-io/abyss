  ...
  <header class="bg-panel h-13 flex items-center justify-between border-b border-b1">
    ...
    <div class="flex-1 flex items-center justify-start min-w-0">
      <img
        ...
        alt="logo"
        class="w-auto h-8 cursor-pointer hover:opacity-80 transition-opacity ml-1"
      />
      <div class="flex-1 flex justify-center px-8 min-w-0">
        <SearchInput
          v-model="search"
          placeholder="Search for projects and applications..."
          max-width="max-w-2xl"
        />
      </div>
    </div>
    <div class="flex items-center justify-end gap-1 pr-4">
      <button
        class="text-t3 hover:text-t1 transition-colors relative p-2 rounded-lg hover:bg-surface"
