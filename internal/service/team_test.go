...
func TestTeamService_UpdateTeam(t *testing.T) {
	t.Run("returns ErrNotFound when team missing", func(t *testing.T) {
		...
		_, err := svc.UpdateTeam(context.Background(), 1, "new", "desc")
	...
	t.Run("updates name and description", func(t *testing.T) {
		...
		got, err := svc.UpdateTeam(context.Background(), 1, "renamed", "new desc")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestTeamService_DeleteTeam(t *testing.T) {
	t.Run("returns ErrNotFound when team missing", func(t *testing.T) {
		...
		err := svc.DeleteTeam(context.Background(), 1)
		if !errors.Is(err, ErrNotFound) {
			t.Fatalf("expected ErrNotFound, got %v", err)
		}
	})

	t.Run("deletes existing team", func(t *testing.T) {
		svc, teamRepo, _, _, _ := newTestTeamService(t)
		team := &model.Team{ID: 1}
		teamRepo.EXPECT().GetByID(context.Background(), uint(1)).Return(team, nil)
		teamRepo.EXPECT().Delete(context.Background(), team).Return(nil)

		if err := svc.DeleteTeam(context.Background(), 1); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
...
func TestTeamService_AddTeamMember(t *testing.T) {
	t.Run("returns ErrNotFound when user does not exist", func(t *testing.T) {
		...
		_, err := svc.AddTeamMember(context.Background(), 1, "ghost", "member")
		if !errors.Is(err, ErrNotFound) {
			t.Fatalf("expected ErrNotFound, got %v", err)
		}
	})

	t.Run("adds member and attaches user", func(t *testing.T) {
		...
		got, err := svc.AddTeamMember(context.Background(), 1, "alice", "member")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
